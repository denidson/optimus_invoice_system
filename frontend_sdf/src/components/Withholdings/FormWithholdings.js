...
import { getInvoices } from '../../services/api_invoices'; // Importa el servicio
import { getWithholdings } from '../../services/apiConfig'; // Importa el servicio
import { createWithholding, showWithholding } from '../../services/apiWithholdings'; // Importa el servicio
...
function WithholdingEditCell({ params, withholdings }) {
  ...
  return (
    <Autocomplete
      ...
      value={withholdings?.find((p) => p.id === params.value) || null}
...
function FormWithholdings() {
  ...
  const [withholdings, setWithholdings] = useState(null);
  ...
  const withholding_id = queryParams.get("id"); // Obtener el ID de la URL
  ...
  const columns = [
    ...
    { field: "factura_afectada_numero", headerName: "Nro. Factura", editable: false },
    { field: "factura_afectada_control", headerName: "Nro. Control", editable: false },
    { field: "factura_afectada_fecha", headerName: "Fecha", editable: false },
    { field: "tipo_documento_afectado", headerName: "Tipo Documento", editable: false },
    { field: "monto_documento", headerName: "Total Neto", editable: false },
    {
    ...
    },
    {
      ...
      renderEditCell: (params) => (
        <WithholdingEditCell params={params} withholdings={withholdings} />
      ),
      // esto asegura que en modo "display" se vea el label en vez del id
      valueFormatter: (params) => {
        const withholding = withholdings?.find((p) => p.id === params);
        return withholding ? `${withholding.descripcion}` : "";
      },
    },
    {
  ...
  useEffect(() => {
    const fetchWithholdings = async () => {
      try {
        const datapts = await getWithholdings();
        setWithholdings(datapts);
        const datacls = await getWithholdings();
        setEndClients(datacls.data);
        if (withholding_id != null){
          const data = await showWithholding(decryptText(withholding_id)); // Llamamos a showWithholding con el ID
          setWithholding(data); // Guardamos los datos de la retencion en el estado
        }else{
          setWithholding({
            id: "#",
            sujeto_retenido_rif: "",
    ...
    fetchWithholdings();
  }, [withholding_id]); // Recarga si el `withholding_id` cambia
  ...
  const taxBase = useMemo(() => {
    if (!withholding?.items) return 0;

    const baseTotal = withholding.items.reduce((acc, item) => {
